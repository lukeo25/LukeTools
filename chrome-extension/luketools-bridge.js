...
(function () {
    ...
    var GUARD = "LukeToolsLocalPanelBridgeLoaded_283";
...
Bridge.openDemBonesMrk2 = function () { return dbm2Host.open(); };
    ...
    // A Clip explicitly marked by Frame Gizmo reopens that tool instead of entering the Clip.
    function installFrameGizmoDoubleClick() {
        if (window.LukeToolsFrameGizmoHost) return window.LukeToolsFrameGizmoHost;
        var marker = "/*LUKE_FRAME_GIZMO:1*/";
        var service = {
            pending: null,
            panel: null,
            opening: false,
            sourceUrl: LT_GITHUB_RAW_BASE + "scripts/Tools/FrameGizmo.txt"
        };
        window.LukeToolsFrameGizmoHost = service;

        service.read = function (object) {
            var scripts = [];
            try { scripts = object && object.scripts ? object.scripts : []; } catch (e0) { scripts = []; }
            for (var i = 0; i < scripts.length; i += 1) {
                if (scripts[i] && s(scripts[i].src).indexOf(marker) >= 0) return true;
            }
            return false;
        };

        service.find = function (object) {
            for (var depth = 0; object && depth < 20; depth += 1, object = object.parent) {
                if (service.read(object)) return object;
            }
            return null;
        };

        service.open = function (clip) {
            if (clip) service.pending = clip;
            try { if (clip) selectObjects(getEditor(), [clip]); } catch (e0) { }
            if (service.panel && service.panel.alive && service.panel.alive()) {
                try { return Promise.resolve(service.panel.resume(service.pending)); } catch (e1) { }
            }
            if (service.opening) return Promise.resolve(false);
            service.opening = true;
            return Promise.resolve(openToolFromUrl(service.sourceUrl, { target: "panel" })).then(function (result) {
                service.opening = false;
        ...
        service.hit = function (event) {
            var project = getProject();
            if (!project || project.playing || !project.view) return null;
            var scope = project.view.paper || window.paper;
            var view = scope && scope.view;
            var canvas = view && (view.element || view._element);
            if (!canvas || event.target !== canvas) return null;
            var rect = canvas.getBoundingClientRect();
            ...
            var hit = scope.project.hitTest(point, {
                fill: true,
                stroke: true,
                segments: false,
                tolerance: 5 / (view.zoom || 1),
                match: function (h) { return !(h.item && h.item.data && h.item.data.wickType === "gui"); }
            });
            if (!hit) return null;
            var cache = window.Wick && window.Wick.ObjectCache;
            for (var item = hit.item; item; item = item.parent) {
                var uuid = item.data && item.data.wickUUID;
                var object = uuid && cache && cache.getObjectByUUID(uuid);
                var found = service.find(object);
        ...
        service.handle = function (event) {
            ...
            var clip = null;
            try { clip = service.hit(event); } catch (e0) { clip = null; }
            if (!clip) return;
            event.preventDefault();
            event.stopImmediatePropagation();
            if (event.type === "mousedown" || !service.lastOpen || Date.now() - service.lastOpen > 500) {
        ...
        document.addEventListener("mousedown", service.handle, true);
        document.addEventListener("dblclick", service.handle, true);
        return service;
    }

    installFrameGizmoDoubleClick();
...
function removeLauncher() {
    ...
    function ensurePanel() {
        ...
        function renderPanelConfig(cfg) { // FIX
            ...
            // A delayed menu refresh must not replace a tool being edited.
            try {
                var activePanel = document.getElementById(PANEL_ID);
        ...
        function autoLoadPanelConfig() {
            // Show the saved menu immediately, then always check for an update.
            // Failed requests leave the saved menu intact.
            var candidates = [];
            ...
            function tryNext() {
                ...
                fetchText(u).then(function (t) {
                    // Parse directly: applyPanelConfigText handles user input by
                    // showing an alert, which is inappropriate for failed URLs.
